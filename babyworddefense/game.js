...
function cancelSpeech(){speechToken++;speaking=false;if(window.speechSynthesis)speechSynthesis.cancel();}
function speak(){if(mode!=='playing'||!target)return;cancelSpeech();const token=speechToken;const u=new SpeechSynthesisUtterance(target);u.lang='en-US';u.rate=.7+grade*.045;const v=voices.find(v=>v.lang==='en-US')||voices[0];if(v)u.voice=v;speaking=true;u.onend=()=>{if(token===speechToken)speaking=false;};u.onerror=e=>{if(token!==speechToken)return;speaking=false;if(e.error!=='canceled'&&e.error!=='interrupted'){pause();$('result').textContent='英文語音無法播放。請確認裝置已安裝英文語音，並開啟音量，再按繼續。';}};speechSynthesis.speak(u);nextPrompt=elapsed+Math.max(2.7,target.length*.12+1.7);}
...
function start(){cancelSpeech();clearBombs();effects=[];score=0;hp=100;elapsed=0;cooldown=0;wave=1;nextSpawn=1.2;nextPrompt=0;target='';hits=0;misses=0;mistakes=0;mode='playing';$('overlay').hidden=true;$('dialog').hidden=true;$('pause').disabled=false;$('listen').disabled=false;try{audio=audio||new(window.AudioContext||window.webkitAudioContext)();audio.resume();}catch(e){}roar();updateHUD();last=performance.now();const ready=new SpeechSynthesisUtterance('Ready');ready.lang='en-US';speechSynthesis.speak(ready);
...
function hit(b){if(mode!=='playing'||!bombs.includes(b))return;if(b.word!==target){score=Math.max(0,score-10);mistakes++;explode(b.x+b.w/2,b.y+24,'−10');remove(b);$('hint').textContent='點錯引爆了！扣 10 分，再仔細聽。';}else{score+=100;hits++;particle(b.x+b.w/2,b.y+20,'#9be6ce','+100');remove(b);sound('hit');$('hint').textContent='答對了！繼續聽下一個單字。';if(!bombs.some(x=>x.word===target))pickTarget();}updateHUD();}
...
function end(){mode='over';cancelSpeech();const record=Math.max(best(),score);try{localStorage.setItem('word-defense-best-'+grade,record);}catch(e){}$('dialog').hidden=false;$('resultLabel').textContent='MISSION COMPLETE';$('dialogTitle').textContent='城市需要你再挑戰';$('result').textContent=`防衛積分 ${score}　｜　最高 ${record}\n守護 ${Math.floor(elapsed)} 秒 · 第 ${wave} 波\n答對 ${hits} 顆 · 漏接 ${misses} 顆 · 點錯 ${mistakes} 次`;$('resume').hidden=true;$('pause').disabled=true;updateHUD();}
function pause(){if(mode!=='playing')return;mode='paused';cancelSpeech();$('dialog').hidden=false;$('resultLabel').textContent='MISSION PAUSED';$('dialogTitle').textContent='休息一下';$('result').textContent='城市已暫停，準備好就繼續守護。';$('resume').hidden=false;updateHUD();}
function resume(){mode='playing';$('dialog').hidden=true;last=performance.now();speak();}
...
function draw(t){ctx.clearRect(0,0,width,height);const sky=ctx.createLinearGradient(0,0,0,height);sky.addColorStop(0,scenePalette[0]);sky.addColorStop(.7,scenePalette[1]);sky.addColorStop(1,scenePalette[2]);ctx.fillStyle=sky;ctx.fillRect(0,0,width,height);
 ...
 // Original procedural kaiju silhouette: horns, shoulders, claws, and glowing eyes.
 ctx.save();ctx.translate(width*.49,height*.59);const scale=width/400;ctx.scale(scale,scale);if(elapsed<roarUntil&&!reduced)ctx.translate(Math.sin(t/35)*3,0);ctx.fillStyle=scenePalette[3];ctx.beginPath();ctx.moveTo(-170,160);ctx.lineTo(-158,12);ctx.lineTo(-126,-63);ctx.lineTo(-95,-75);ctx.lineTo(-88,-147);ctx.lineTo(-65,-121);ctx.lineTo(-40,-165);ctx.lineTo(-13,-151);ctx.lineTo(15,-168);ctx.lineTo(39,-140);ctx.lineTo(75,-168);ctx.lineTo(70,-103);ctx.lineTo(112,-69);ctx.lineTo(154,7);ctx.lineTo(178,160);ctx.closePath();ctx.fill();ctx.beginPath();ctx.ellipse(0,65,125,170,0,0,Math.PI*2);ctx.fill();ctx.fillStyle=elapsed<roarUntil?'#ffac7a':'#ffe68d';ctx.shadowColor='#f7be76';ctx.shadowBlur=elapsed<roarUntil?24:8;ctx.beginPath();ctx.moveTo(-57,-97);ctx.lineTo(-15,-89);ctx.lineTo(-24,-81);ctx.lineTo(-51,-84);ctx.fill();ctx.beginPath();ctx.moveTo(14,-89);ctx.lineTo(56,-97);ctx.lineTo(50,-84);ctx.lineTo(23,-81);ctx.fill();ctx.shadowBlur=0;if(elapsed<roarUntil){ctx.fillStyle='#050d19';ctx.beginPath();ctx.ellipse(0,-51,24,22,0,0,Math.PI*2);ctx.fill();}ctx.restore();
