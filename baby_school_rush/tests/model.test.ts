...
import {Run, SPEED, BALL_ROLL_SPEED, obstacleDistance} from '../src/model.ts';
...
test('puddles, rolling balls and boxes are low obstacles that require a jump',()=>{const r=new Run();r.obstacles=[{at:1,kind:'puddle',hit:false},{at:3,kind:'ball',hit:false},{at:5,kind:'box',hit:false}];tick(r,6);assert.equal(r.hits,3);});
test('ball enters from farther right and reaches the player at its authored beat',()=>{assert.equal(obstacleDistance(10,'ball',8),2*(SPEED+BALL_ROLL_SPEED));assert.equal(obstacleDistance(10,'cone',8),2*SPEED);assert.equal(obstacleDistance(10,'ball',10),0);});
